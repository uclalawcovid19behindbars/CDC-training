...
## get the labels to the data
## for more on working with labelled data: https://www.pipinghotdata.com/posts/2020-12-23-leveraging-labelled-data-in-r/
...
merged_df <- filtered_data %>% 
  left_join(bjs, 
            by = c("BJS.ID" = "Facility ID #1")) ## specify the variable in common between the two data sets

## view the merged data set
#View(merged_df)
```

Now, we create some plots showing facility-level infection rates by facility operator type.
...
Next, we leverage the BJS data specification of facility operator to group facilities by private and non-private status.


```{r, message=FALSE, warning=FALSE}
## NOTE: not sure if we should include this section at all! 
...
## Comparing infection rates to the non-incarcerated population

TO DO: Choose a facility from California historical data and make a plot comparing it to county-level pop infection rates
